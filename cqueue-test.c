...
void CQueueBasicTest() {
	...
	//verify that get/peek return EMPTYQUEUE when queue is empty
	verify("get on empty set", CQueueGet(cq), EMPTYQUEUE, __func__, __LINE__, true);
	verify("peek on empty set", CQueuePeek(cq), EMPTYQUEUE, __func__, __LINE__, true);
